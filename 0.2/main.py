...
'''============================= GLOBAL VARIABLE ========================='''
debugFPS = 0
debug0 = 0 # debug size # actual not really work FIX IT!!!
...
class Game:
    ...
    def update(self):
        ...
        if  debug0:
            print(self.runChoice.__sizeof__(), 'total Sizee  = ', self.__sizeof__())
        
    def draw(self):
        # will be no image until loading finish
        if self.runChoice.isLoad and self.runChoice.isBegin:
            self.runChoice.draw()
        else : # next time move all loading function to loadingscreen class
            LoadingScreen(self).draw()
